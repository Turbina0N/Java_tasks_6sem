...
public class HttpServerTest {
    ...
    private static HttpHandler mockHandler;
    ...
    public static void setUp() throws InterruptedException {
        ...
        mockHandler = mock(HttpHandler.class);
    ...
    public void testAddHandler() {
        server.addHandler("/test", "GET", mockHandler);
        HttpHandler handler = server.getHandlers().get("GET /test");
    ...
    public void testDefaultHandlers() {
        ...
        HttpHandler getHandler = server.getHandlers().get("GET /");
        assertNotNull(getHandler);

        HttpHandler postHandler = server.getHandlers().get("POST /");
        assertNotNull(postHandler);

        HttpHandler putHandler = server.getHandlers().get("PUT /");
        assertNotNull(putHandler);

        HttpHandler patchHandler = server.getHandlers().get("PATCH /");
        assertNotNull(patchHandler);

        HttpHandler deleteHandler = server.getHandlers().get("DELETE /");
    ...
    public void testHandlePostRequestWithJson() throws IOException {
        try (Socket clientSocket = new Socket("localhost", 8080)) {
            ...
            System.out.println(responseStr);
            assertTrue(responseStr.contains("HTTP/1.1 200 OK"));
            assertTrue(responseStr.contains("{\"message\":\"Hello World\"}"));
        }
    }
    ...
    public void testHandlePutMultipartFormDataRequest() throws IOException {
        try (Socket clientSocket = new Socket("localhost", 8080)) {
            // Send POST request with multipart/form-data body
            OutputStream os = clientSocket.getOutputStream();
            ...
            assertTrue(responseStr.contains("This is a test file."));
        }
    }
